<?php

namespace Modules\LOMBRISOFT\Database\Seeders;

use App\Models\User;
use Illuminate\Database\Seeder;
use Modules\SICA\Entities\Person;



class UsersTableSeeder extends Seeder
{
    public function run()
    {
        $person = Person::where('document_number', 1079172956)->first();
        User::updateOrCreate(['nickname' => 'Nardila'], [
            'person_id' => $person->id,
            'email' => 'ardilanicolas71@gmail.com'            //Password: Niar2956
        ]);
        $person = Person::where('document_number', 1079174923)->first();
        User::updateOrCreate(['nickname' => 'Bsanchez'], [
            'person_id' => $person->id,
            'email' => 'sanchezbreiner2014@gmail.com'            //Password: Brsa4923
        ]);
        $person = Person::where('document_number', 1079173032)->first();
        User::updateOrCreate(['nickname' => 'pabloc'], [
            'person_id' => $person->id,
            'email' => 'paceburo21@gmail.com'            //Password: Pabu3032
        ]);
        $person = Person::where('document_number', 7719513)->first();
        User::updateOrCreate(['nickname' => 'eduarp'], [
            'person_id' => $person->id,
            'email' => 'epastranag@sena.edu.co'            //Password: Edpa9513
        ]);
    }
}
